...
object BasicStatistics extends StrictLogging {
  ...
  def customCountMissValues(e: Column): Column = {
    ...
    val unionMissingValues: Column = sum(
      when(
        e.isNull
        || e === ""
        || e === " "
...
case class DiscreteMetric(name: String, function: ((Column, DataFrame)) => Column)
  ...
  val discreteMetrics: List[DiscreteMetric] = List(Category,CountDistinct, CountDiscrete, Frequencies, CountMissValuesDiscrete)
